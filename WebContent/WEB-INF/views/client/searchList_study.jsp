	...
	<%@ include file="apart_body_header.jsp" %>
		...
		<fieldset class="search">       	
       		<div class="search" style="text-align: -webkit-center;">
        		<label>Search In Total</label><br>
       			<input type="text" id="search" name="search" value="${param.search }">
       			<input type="button" id="src_btn" class="btn btn-primary" value="Search">
       		</div>
        </fieldset>
       	</section>
       		<br><br>
			<div class="searchDetail">
				<legend>Search In Detail</legend>
				<select name="searchType" id="searchType">
					<option value="1" <c:if test="${param.searchType == '1' }">selected</c:if>>Title</option>
					<option value="2" <c:if test="${param.searchType == '2' }">selected</c:if>>Content</option>
					<option value="3" <c:if test="${param.searchType == '3' }">selected</c:if>>Date</option>
				</select>
