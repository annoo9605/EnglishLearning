...
<!-- 			<a href="#"><i class="fa fa-angle-double-left"></i></a></span> -->
			<a href="./studyCalendar.do?year=${dateList[0].prevYear }&month=${dateList[0].prevMonth }">  <img src='/resources/images/arrow-back-icon.png' style="padding-right: 30px;"></a>
			...
			<a href="./studyCalendar.do?year=${dateList[0].nextYear }&month=${dateList[0].nextMonth}">  <img src='/resources/images/arrow-next-3-icon.png' style="padding-left: 50px;"></a>
	       			...
	       			<c:choose>
	       				<c:when test="${list.week == 1 }">
	       					<tr>
	       						...
	       						${list.date}
       							<c:forEach var="row" items="${list.point}">
	       							<br/><a href="./retrieve.do?activity=${row.ACTIVITY}&Year=${list.thisYear}&Month=${list.thisMonth}&Date=${list.date}">${row.ACTIVITY} : ${row.POINT}</a>
<%-- 	       							<br/><a href="./retrieve.do?activity=${row.ACTIVITY}">${row.ACTIVITY} : ${row.POINT}</a> --%>
	       						</c:forEach>
	       						</td>
...
<%-- 	       							<c:forEach begin="1" end="${list.week - 1}"> --%>
	       						...
	       						${list.date}
	       						<c:forEach var="row" items="${list.point}">
	       							<br/><a href="./retrieve.do?activity=${row.ACTIVITY}&Year=${list.thisYear}&Month=${list.thisMonth}&Date=${list.date}" style="color: crimson; font-family: fantasy;">
<%-- 	       							<br/><a href="./retrieve.do?activity=${row.ACTIVITY}" style="color: crimson; font-family: fantasy;"> --%>
    								<c:choose>
